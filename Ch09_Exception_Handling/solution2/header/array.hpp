...
namespace mine {
	...
	template<class type>
	array<type>::array(size_t max_size)
		: body(0), length(0), limit(max_size)
	{
	}
	...
	type &array<type>::operator[](size_t index)
	{
		if (int(index) < 0)
		{
			std::domain_error de("Negative subscript");
			throw de;
		}
		else
			if (index >= size())
	...
	{
		if (limit && new_size > limit)
			throw std::length_error("attempt to resize above maximum possible");
	...
	type &array<type>::subscript(size_t index) const
	{
		if (ptrdiff_t(index) < 0)
			throw std::domain_error("possible negative index");
