...
namespace TimesDates {
	...
	void setMockClock(const volatile ClockAsBCD * clock, TimeDate time) {
		ClockAsBCD & hardware = const_cast<ClockAsBCD &>(*clock);
		hardware._year = toBCD(time.year());
		hardware._month = toBCD(time.month());
		hardware._day = toBCD(time.day());
		hardware._hr = toBCD(time.hrs());
		hardware._min = toBCD(time.mins());
	}
