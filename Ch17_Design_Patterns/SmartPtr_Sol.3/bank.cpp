...
unique_ptr<account_impl> bank::create_account(const string &number) const
{
	for (size_t i = 0; i < table_size; i++)
		if (table[i].account_number == number)
			return make_unique<account_impl>(table[i].account_number, table[i].account_balance);
	
	// If account number not found, create a new account with
	// specified account number
	return make_unique<account_impl>(number, 0.00);
}
